...
4. **Exact token added**:
`shell.scenes.space.star1Fill`

5. **Exact raw CSS variable replaced**:
`var(--sa-scene-space-star1)` for fill/background uses only.

6. **Confirmation that only SpaceScene star 1 fill/background was migrated**:
Confirmed. The star 1 shadow was explicitly preserved as requested since it utilizes the same css variable format.

7. **Confirmation that SpaceScene star 2 was not migrated**:
Confirmed. SpaceScene star 2 remains functionally identical using its raw `bg-[var(--sa-scene-space-star2)]`.

8. **Confirmation that SpaceScene star 3 was not migrated**:
Confirmed. SpaceScene star 3 uses raw `bg-[var(--sa-scene-space-star3)]`.

9. **Confirmation that SpaceScene nebula was not migrated**:
Confirmed. SpaceScene nebula continues to use `bg-[var(--sa-scene-space-nebula)]`.

10. **Confirmation that NightScene was not modified**:
Confirmed. 

11. **Confirmation that SkyScene was not modified**:
Confirmed.

12. **Confirmation that SunsetScene was not modified**:
Confirmed.

13. **Confirmation that gameplay files were not modified**:
Confirmed. Gameplay was not touched.
...
Confirmed. No Distractors, StarAnimationEffect, or SparkleEffect exist in the container.

16. **Build/lint/test results**:
Build: PASS 
Lint: PASS
Vitest resolver tests: PASS
Vitest registry tests: PASS

17. **Manual smoke QA results**:
All specified visuals appear untouched, testing succeeds.
