# GAMEPLAY-STABILITY-1E: START_GAME Ref Cleanup Only
...
## Full Contents of Modified Source File
### `src/hooks/useGameLogic.ts`
...
## Exact Location of Cleanup
Inside the `startGame` callback defined in `src/hooks/useGameLogic.ts`, right before processing the `activeConfig`, `learningMode`, and dispatching `START_GAME`.

## Confirmations
- Only `retryStep` and `stepTries` were intentionally reset.
- No other refs were modified.
- Reducer was not modified.
- SkinLab was not modified.
- Visibility guard behavior was not modified.
...
- FAILED STEP → HOME → NEW GAME SMOKE TEST: PASS
- CENTER-CIRCLE GAMEPLAY SMOKE TEST: PASS
- VISIBILITY PAUSE GUARD SMOKE TEST: PASS

## Recommended Next Phase
Continue moving to GAMEPLAY-STABILITY-2 Phase for stability or functional enhancements as needed for gameplay semantics and failsafes.
