...
# 6. Update render
# We need to replace function render() until drawForegroundParallax()
# Wait, drawForegroundParallax() could be tricky to match correctly
render = """
    function render() {
      ctx.save();
      ctx.setTransform(dpr, 0, 0, dpr, 0, 0);
...
"""
code = re.sub(r'function render\(\) \{[\s\S]*?drawForegroundParallax\(\);', render.strip(), code)
...
build_circuit_path = """
    function buildCircuitPath(from: any, to: any, destinationPlatform: any = null) {
      ...
      // If no safe route is found, throw an error instead of going out of bounds
      console.warn("No clear circuit path found to platform!");
      return buildSteppedRoute(
        from,
        to,
        platform,
        preferred, // fallback to preferred even if not clear
      );
    }
"""
...
code = re.sub(r'const crossX =.*?;\n\s+const midCrossY', r'const crossX = corridor.center;\n      const midCrossY', code, flags=re.DOTALL)
code = re.sub(r'const crossX =.*?;(?=\n.*?const midCrossY)', 'const crossX = corridor.center;', code, flags=re.DOTALL)
